...
/* ─── Constants ──────────────────────────────────────────────── */
const PROC_STATUSES = ["PENDING", "PROCESSING", "COMPLETED", "FAILED", "DUPLICATE"];
const PROC_LABELS = { PENDING: "Pending", PROCESSING: "Processing", COMPLETED: "Completed", FAILED: "Failed", DUPLICATE: "Duplicate" };
...
const MOCK_RECORDS = [
    { id: "RAW-10042", srcId: "SF-CRM-00142", entity: "CUSTOMER", source: "Salesforce CRM", run: "RUN-0042", status: "COMPLETED", receivedAt: "2026-05-11 10:01:03", payload: { customerName: "ABC Pharma Pvt. Ltd.", emailId: "contact@abc.com", phone: "+91-9900112233", billingAddress: "Andheri East, Mumbai 400069", tier: "PREMIUM", creditLimit: 500000 } },
    { id: "RAW-10041", srcId: "SF-CRM-00143", entity: "CUSTOMER", source: "Salesforce CRM", run: "RUN-0042", status: "COMPLETED", receivedAt: "2026-05-11 10:01:04", payload: { customerName: "Delta Logistics", emailId: "ops@delta.io", phone: "+91-9812345678", billingAddress: "Whitefield, Bengaluru 560066", tier: "STANDARD" } },
    { id: "RAW-10040", srcId: "SAP-PROD-7721", entity: "PRODUCT", source: "SAP ERP Core", run: "RUN-0041", status: "PROCESSING", receivedAt: "2026-05-11 09:46:22", payload: { productName: "Industrial Pump XR-900", sku: "PUMP-XR-900-IND", category: "MACHINERY", uom: "UNIT", basePrice: 74500, currency: "INR" } },
    { id: "RAW-10039", srcId: "ORA-ACC-9910", entity: "ACCOUNT", source: "Oracle Finance", run: "RUN-0040", status: "COMPLETED", receivedAt: "2026-05-11 08:30:45", payload: { accountName: "Corporate Main A/C", accountNumber: "ACC-9910-CORP", currency: "INR", accountType: "CURRENT", bankName: "HDFC Bank", ifscCode: "HDFC0001234" } },
    { id: "RAW-10038", srcId: "VP-SUP-00334", entity: "SUPPLIER", source: "Vendor Portal", run: "RUN-0039", status: "COMPLETED", receivedAt: "2026-05-11 08:01:12", payload: { supplierName: "Mehta Precision Parts", taxId: "GSTIN29AA2150P1ZQ", contactEmail: "supply@mehta.co.in", paymentTerms: "NET30", currency: "INR" } },
    { id: "RAW-10037", srcId: "WD-CUST-00881", entity: "CUSTOMER", source: "Workday HRMS", run: "RUN-0038", status: "FAILED", receivedAt: "2026-05-10 17:23:05", payload: { customerName: "", emailId: "noname@workday.com", phone: "+91-8866554433", billingAddress: "Connaught Place, New Delhi 110001" } },
    { id: "RAW-10036", srcId: "SAP-PROD-7689", entity: "PRODUCT", source: "SAP ERP Core", run: "RUN-0041", status: "DUPLICATE", receivedAt: "2026-05-11 09:46:35", payload: { productName: "Industrial Pump XR-900", sku: "PUMP-XR-900-IND", category: "MACHINERY", uom: "UNIT", basePrice: 74500 } },
    { id: "RAW-10035", srcId: "LG-CUST-01122", entity: "CUSTOMER", source: "Legacy Master DB", run: "RUN-0037", status: "COMPLETED", receivedAt: "2026-05-10 14:01:09", payload: { customerName: "Pinnacle Exports Ltd.", emailId: "ceo@pinnacle.in", phone: "+91-9911223344", billingAddress: "MIDC Andheri, Mumbai 400093", tier: "ENTERPRISE", creditLimit: 2000000 } },
    { id: "RAW-10034", srcId: "SF-CUST-00889", entity: "CUSTOMER", source: "Salesforce CRM", run: "RUN-0042", status: "PENDING", receivedAt: "2026-05-11 10:01:08", payload: { customerName: "BlueSky Technologies", emailId: "info@bluesky.io", phone: "+91-8877665544", billingAddress: "Hinjewadi, Pune 411057" } },
];

MOCK_RECORDS.forEach(r => { r.checksum = makeChecksum(r.id + r.srcId); });
...
/* ─── Payload Viewer Modal ───────────────────────────────────── */
function PayloadModal({ record, onClose }) {
        ...
        <div className="rl-modal-overlay" onClick={onClose}>
            <div className="rl-modal" onClick={e => e.stopPropagation()} role="dialog" aria-modal="true">
                <div className="rl-modal__header">
                    <div>
                        <h2 className="rl-modal__title">{record.srcId} — {record.entity}</h2>
                        <div className="rl-modal__sub">{record.id} · {record.source}</div>
                    </div>
                    <button className="rl-modal__close" onClick={onClose}>✕</button>
                </div>

                <div className="rl-modal-tabs">
                    {[["payload", "Payload"], ["metadata", "Metadata"]].map(([key, label]) => (
                        <button key={key} className={`rl-modal-tab${tab === key ? " rl-modal-tab--active" : ""}`} onClick={() => setTab(key)}>
                            {label}
                        </button>
                    ))}
                </div>
...
export default function RawLanding() {
    ...
    const [viewTab, setViewTab] = useState("payload");
    ...
    const openModal = (rec, tab = "payload") => { setViewRecord(rec); setViewTab(tab); };
    ...
    return (
        <div className="rl-page">
            ...
            <div className="rl-summary-row">
                <div className="rl-summary-card"><span className="rl-summary-card__value">{records.length}</span><span className="rl-summary-card__label">Total Records</span></div>
                <div className="rl-summary-card rl-summary-card--green"><span className="rl-summary-card__value">{completed}</span><span className="rl-summary-card__label">Completed</span></div>
                <div className="rl-summary-card" style={{ borderTopColor: "var(--blue-500)" }}><span className="rl-summary-card__value">{processing}</span><span className="rl-summary-card__label">Processing</span></div>
                <div className="rl-summary-card rl-summary-card--red"><span className="rl-summary-card__value">{failed}</span><span className="rl-summary-card__label">Failed</span></div>
                <div className="rl-summary-card rl-summary-card--amber"><span className="rl-summary-card__value">{duplicates}</span><span className="rl-summary-card__label">Duplicates</span></div>
            </div>
            ...
            <div className="rl-table-card">
                <div className="rl-table-toolbar">
                    <div className="rl-search-wrap">
                        ...
                        <input className="rl-search-input" placeholder="Search by raw ID, source ID, entity…" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
                    </div>
                    <div className="rl-filter-row">
                        <select className="rl-select" value={filterSource} onChange={e => { setFilterSource(e.target.value); setPage(1); }}>
                            <option value="ALL">All Sources</option>
                            {SOURCES.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                        <select className="rl-select" value={filterEntity} onChange={e => { setFilterEntity(e.target.value); setPage(1); }}>
                            <option value="ALL">All Entities</option>
                            {ENTITIES.map(e => <option key={e} value={e}>{e}</option>)}
                        </select>
                        <select className="rl-select" value={filterStatus} onChange={e => { setFilterStatus(e.target.value); setPage(1); }}>
                            <option value="ALL">All Statuses</option>
                            {PROC_STATUSES.map(s => <option key={s} value={s}>{PROC_LABELS[s]}</option>)}
                        </select>
                        <select className="rl-select" value={filterRun} onChange={e => { setFilterRun(e.target.value); setPage(1); }}>
                            <option value="ALL">All Runs</option>
                            {MOCK_RUNS.map(r => <option key={r} value={r}>{r}</option>)}
                        </select>
                        <span className="rl-count-label">{filtered.length} record{filtered.length !== 1 ? "s" : ""}</span>
                    </div>
                </div>
                ...
                <div className="rl-table-wrap">
                    <table className="rl-table">
                        ...
                        <tbody>
                            {paginated.length === 0 ? (
                                <tr><td colSpan={8} className="rl-table-empty"><span>🗄</span><p>No raw records found</p></td></tr>
                            ) : paginated.map(rec => (
                                <tr key={rec.id} className="rl-table-row">
                                    <td><code className="rl-raw-id">{rec.id}</code></td>
                                    <td><span className="rl-src-id">{rec.srcId}</span></td>
                                    <td><span className="rl-entity-chip">{rec.entity}</span></td>
                                    <td style={{ fontSize: 13, color: "var(--text-secondary)", fontWeight: 500 }}>{rec.source}</td>
                                    <td>
                                        <span className={`rl-proc-status rl-proc-status--${rec.status}`}>
                                            {PROC_LABELS[rec.status]}
                                        </span>
                                    </td>
                                    <td><span className="rl-ts">{rec.receivedAt}</span></td>
                                    <td><span className="rl-checksum" title={rec.checksum}>{rec.checksum}</span></td>
                                    <td>
                                        <div className="rl-action-row">
                                            <button className="rl-action-btn rl-action-btn--primary" onClick={() => openModal(rec, "payload")}>View Payload</button>
                                            <button className="rl-action-btn" onClick={() => openModal(rec, "metadata")}>Metadata</button>
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    a.href = url; a.download = `${rec.id}.json`; a.click();
                ...
                <div className="rl-pagination">
                    ...
                    <div className="rl-pagination__btns">
                        <button className="rl-page-btn" onClick={() => setPage(p => p - 1)} disabled={page === 1}>←</button>
                        {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                            <button key={p} className={`rl-page-btn${p === page ? " rl-page-btn--active" : ""}`} onClick={() => setPage(p)}>{p}</button>
                        ))}
                        <button className="rl-page-btn" onClick={() => setPage(p => p + 1)} disabled={page === totalPages}>→</button>
                    </div>
                </div>
            </div>
